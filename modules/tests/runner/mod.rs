...
use ibc::{
	core::{
		...
		ics26_routing::{error as routing_error, msgs::Ics26Envelope},
	},
	mock::{
	...
	relayer::ics18_relayer::{context::Ics18Context, error as relayer_error},
	signer::Signer,
...
impl IbcTestRunner {
	...
	pub fn init_chain_context(&mut self, chain_id: String, initial_height: Height) {
		...
		let ctx =
			MockContext::new(chain_id_struct, HostType::Mock, max_history_size, initial_height);
	...
	fn signer() -> Signer {
		"cosmos1wxeyh7zgn4tctjzs0vtqpc6p5cxq5t2muzl7ng".parse().unwrap()
	}
	...
	pub fn proofs(height: Height) -> Proofs {
		...
		Proofs::new(object_proof, client_proof, consensus_proof, other_proof, height)
	...
	pub fn check_chain_states(&self, chains: HashMap<String, Chain>) -> bool {
		chains.into_iter().all(|(chain_id, chain)| {
			...
			let clients_match = chain.clients.into_iter().all(|(client_id, client)| {
				...
				//       exist in the model)
				let consensus_states_match = client
					.heights
					.into_iter()
					.all(|height| ctx.consensus_state(&Self::client_id(client_id), height).is_ok());
			...
			let connections_match =
				chain.connections.into_iter().all(|(connection_id, connection)| {
						...
						ctx.connection_end(&Self::connection_id(connection_id))
					{
						...
						let counterparty_client_ids = *connection_end.counterparty().client_id() ==
							Self::client_id(connection.counterparty_client_id.unwrap());

						// counterparty connection ids must match
						let counterparty_connection_ids = connection_end
							.counterparty()
	...
	pub fn apply(&mut self, action: Action) -> Result<(), relayer_error::Error> {
		match action {
			...
			Action::Ics02CreateClient { chain_id, client_state, consensus_state } => {
				// get chain's context
			...
			Action::Ics02UpdateClient { chain_id, client_id, header } => {
				// get chain's context
				let ctx = self.chain_context_mut(chain_id);
			...
			Action::Ics07UpgradeClient { chain_id, client_id, header } => {
				// get chain's context
				let ctx = self.chain_context_mut(chain_id);
...
impl modelator::step_runner::StepRunner<Step> for IbcTestRunner {
	fn initial_step(&mut self, step: Step) -> Result<(), String> {
		...
		assert_eq!(step.action_outcome, ActionOutcome::None, "unexpected action outcome");
