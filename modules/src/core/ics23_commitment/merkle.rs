...
/// The encoding and decoding shouldn't fail since ics23::CommitmentProof and
/// ibc_proto::ics23::CommitmentProof should be the same Ref. <https://github.com/informalsystems/ibc-rs/issues/853>
impl<H> From<RawMerkleProof> for MerkleProof<H> {
...
impl<H> From<MerkleProof<H>> for RawMerkleProof {
	fn from(proof: MerkleProof<H>) -> Self {
		Self {
			proofs: proof
				.proofs
				.into_iter()
	...
	H: HostFunctionsProvider,
{
	...
	pub fn verify_non_membership(
		...
		// keys are represented from root-to-leaf
		let key = keys.key_path.get(num - 1).ok_or_else(Error::invalid_merkle_proof)?;
		match &proof.proof {
...
//     - structure that includes a RawProofOps in its only `proof` field. #[derive(Clone, PartialEq,
//       ::prost::Message)] pub struct MerkleProof { #[prost(message, optional, tag="1")] pub proof:
//       ::core::option::Option<::tendermint_proto::crypto::ProofOps>, }
//  - Vec<u8>: RawMerkleProof is not explicitly used but, serialized as Vec<u8>, it is included in
//    all handshake messages that require proofs (i.e. all except the two `OpenInit`), and also in
//    all queries that require proofs
//  - MerkleProof: Domain type for RawMerkleProof, currently not used and identical to
//    RawMerkleProof. This will change with verification implementation.
...
//  - commitment.rs: Vec<u8> <-> CommitmentProof CommitmentProof <-> RawMerkleProof
//  - merkle.rs: RawMerkleProof <-> MerkleProof
//  - tendermint-rs/src/merkle/proof.rs: TmProof <-> RawProofOps
//  - cosmos.rs:abci_query() converts from query proof to Merkle proof: RawProofOps =>
//    RawMerkleProof
//
