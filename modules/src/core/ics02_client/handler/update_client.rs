...
use crate::{
	...
	core::{
		ics02_client::{
			client_def::{AnyClient, ClientDef, ConsensusUpdateResult},
...
mod tests {
	...
	use crate::{
		...
		mock::{
			client_state::MockClientState, context::MockContext, header::MockHeader, host::HostType,
		},
		prelude::*,
		test_utils::{get_dummy_account_id, Crypto},
		timestamp::Timestamp,
		Height,
	};
	...
	fn test_update_synthetic_tendermint_client_adjacent_ok() {
		...
		let msg =
			MsgUpdateAnyClient { client_id: client_id.clone(), header: latest_header, signer };
	...
	fn test_update_synthetic_tendermint_client_lower_height() {
		...
		let msg = MsgUpdateAnyClient { client_id, header: latest_header, signer };
