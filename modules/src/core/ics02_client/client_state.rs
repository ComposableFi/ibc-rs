...
use crate::{
	...
	core::{
		ics02_client::{client_type::ClientType, error::Error, trust_threshold::TrustThreshold},
		ics24_host::{
...
impl From<AnyClientState> for Any {
	fn from(value: AnyClientState) -> Self {
		match value {
			...
			#[cfg(any(test, feature = "ics11_beefy"))]
			AnyClientState::Beefy(value) =>
				Any { type_url: BEEFY_CLIENT_STATE_TYPE_URL.to_string(), value: value.encode_vec() },
			#[cfg(any(test, feature = "ics11_beefy"))]
			AnyClientState::Near(_) =>
				Any { type_url: BEEFY_CLIENT_STATE_TYPE_URL.to_string(), value: value.encode_vec() },
			#[cfg(any(test, feature = "mocks"))]
			AnyClientState::Mock(value) =>
				Any { type_url: MOCK_CLIENT_STATE_TYPE_URL.to_string(), value: value.encode_vec() },
		}
	}
}
	...
	fn upgrade(
		...
		match self {
			...
			AnyClientState::Near(near_state) => near_state
				...
				.wrap_any(),
			#[cfg(any(test, feature = "mocks"))]
			AnyClientState::Mock(mock_state) => mock_state.upgrade(upgrade_height, (), chain_id).wrap_any(),
		}
	}
...
impl IdentifiedAnyClientState {
	pub fn new(client_id: ClientId, client_state: AnyClientState) -> Self {
		IdentifiedAnyClientState { client_id, client_state }
	}
}
...
mod tests {
	...
	use crate::{
		...
		core::ics02_client::client_state::AnyClientState,
	};
