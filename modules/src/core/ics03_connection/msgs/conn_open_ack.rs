...
impl TryFrom<RawMsgConnectionOpenAck> for MsgConnectionOpenAck {
	...
	fn try_from(msg: RawMsgConnectionOpenAck) -> Result<Self, Self::Error> {
		...
		Ok(Self {
			connection_id: msg.connection_id.parse().map_err(Error::invalid_identifier)?,
...
#[cfg(test)]
pub mod test_util {
	use crate::{core::ics02_client::client_state::AnyClientState, prelude::*};
	...
	fn parse_connection_open_ack_msg() {
		...
		let tests: Vec<Test> = vec![
			...
			Test {
				...
				raw: RawMsgConnectionOpenAck { version: None, ..default_ack_msg.clone() },
				want_pass: false,
			},
			Test {
