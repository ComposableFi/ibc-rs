...
use crate::{
	...
	events::{extract_attribute, Error as EventError, RawObject},
...
impl FromStr for Sequence {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		Ok(Self::from(
			s.parse::<u64>()
				.map_err(|e| Error::invalid_string_as_sequence(s.to_string(), e))?,
		))
	}
}
...
impl TryFrom<RawPacket> for Packet {
	...
	fn try_from(raw_pkt: RawPacket) -> Result<Self, Self::Error> {
		...
		Ok(Packet {
			...
			destination_port: raw_pkt.destination_port.parse().map_err(Error::identifier)?,
			destination_channel: raw_pkt.destination_channel.parse().map_err(Error::identifier)?,
			data: raw_pkt.data,
...
mod tests {
	...
	fn serialize_and_deserialize_packet() {
		let packet = Packet { data: vec![5; 32], ..Default::default() };
