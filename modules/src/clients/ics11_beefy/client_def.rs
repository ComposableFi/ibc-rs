...
use crate::{
	clients::{
		host_functions::{HostFunctionsManager, HostFunctionsProvider},
		ics11_beefy::{
...
impl<HostFunctions: HostFunctionsProvider> ClientDef for BeefyClient<HostFunctions> {
	...
	fn update_state(
		...
		let mut client_state = client_state.from_header(header.clone()).map_err(Error::beefy)?;
		...
		if let Some(parachain_headers) = header.headers_with_proof {
			for header in parachain_headers.headers {
				...
				parachain_cs_states
					.push((height, AnyConsensusState::Beefy(ConsensusState::from_header(header)?)))
			}
		}
...
fn verify_delay_passed(
	...
	let processed_time = ctx.client_update_time(client_id, height).map_err(|_| {
		Error::beefy(BeefyError::processed_time_not_found(client_id.clone(), height))
	})?;
	let processed_height = ctx.client_update_height(client_id, height).map_err(|_| {
		Error::beefy(BeefyError::processed_height_not_found(client_id.clone(), height))
	})?;
