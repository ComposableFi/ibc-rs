	...
	H: HostFunctionsProvider,
{
	...
	fn verify_header(
		...
		if header.height().revision_number != client_state.chain_id.version() {
			return Err(Ics02Error::tendermint_handler_error(Error::mismatched_revisions(
		...
		match verdict {
			...
			Verdict::Invalid(detail) =>
				return Err(Ics02Error::tendermint_handler_error(Error::verification_error(detail))),
		}
