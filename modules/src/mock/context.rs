...
use crate::{
	clients::{
		ics07_tendermint::client_state::test_util::get_dummy_tendermint_client_state,
		ics11_beefy::{
			client_state::test_util::get_dummy_beefy_state,
			consensus_state::test_util::get_dummy_beefy_consensus_state,
		},
	},
	core::{
		ics02_client::{
		...
		ics26_routing::{
			...
			handler::{deliver, dispatch, MsgReceipt},
			msgs::Ics26Envelope,
		},
	},
	events::IbcEvent,
...
impl Default for MockContext {
	fn default() -> Self {
		Self::new(ChainId::new("mockgaia".to_string(), 0), HostType::Mock, 5, Height::new(0, 5))
	}
}
...
impl MockContext {
	...
	pub fn with_client_parametrized(
		...
		let (client_state, consensus_state) = match client_type {
			...
			ClientType::Beefy => (Some(get_dummy_beefy_state()), get_dummy_beefy_consensus_state()),
